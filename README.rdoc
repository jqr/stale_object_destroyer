...
Stale Object Destroyer provides an easy way for you to start the entire request over to ensure no stale objects will be operated on. It handles this entirely inside of Rails though, so no additional client requests will be made. As far as your code will know, the user just retried the request a few times in hopes of success.
